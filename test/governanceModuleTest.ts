...
describe('proposalModule:', () => {
  ...
  it('can vote on multiple proposals', async () => {
    ...
    await govToken.connect(wallet_1).approve(linearVoting.address, ethers.BigNumber.from('500000000000000000'))
    await linearVoting.connect(wallet_1).delegateVotes(wallet_1.address, ethers.BigNumber.from('500000000000000000'))
    await govToken.connect(wallet_2).approve(linearVoting.address, ethers.BigNumber.from('500000000000000000'))
    await linearVoting.connect(wallet_2).delegateVotes(wallet_2.address, ethers.BigNumber.from('500000000000000000'))
    let transferCall = buildContractCall(govToken, "transfer", [wallet_3.address, 1000], await safe.nonce())
    await proposalModule.submitModularProposal(govToken.address, 0, transferCall.data)
    await proposalModule.connect(wallet_1).submitModularProposal(govToken.address, 0, transferCall.data)
    await proposalModule.connect(wallet_2).submitModularProposal(govToken.address, 0, transferCall.data)
    ...
    await network.provider.send("evm_increaseTime", [60])
    await proposalModule.startModularQueue(0)
    await proposalModule.startModularQueue(1)
    await proposalModule.startModularQueue(2)
    await network.provider.send("evm_increaseTime", [60])
    await proposalModule.executeModularProposal(0)
    await proposalModule.executeModularProposal(1)
    await proposalModule.executeModularProposal(2)
    expect(await govToken.balanceOf(wallet_3.address)).to.equal(ethers.BigNumber.from('1000000000000003000'))
  })
  ...
  it('cannot undelegate if open proposals have been voted on', async () => {
    ...
    expect(delegation.total).to.equal(ethers.BigNumber.from('500000000000000000'))
    ...
    expect(await govToken.balanceOf(wallet_0.address)).to.equal(ethers.BigNumber.from('49996500000000000000000'))
    await linearVoting.undelegateVotes(wallet_0.address, ethers.BigNumber.from('500000000000000000'))
    ...
    expect(await govToken.balanceOf(wallet_0.address)).to.equal(ethers.BigNumber.from('49997000000000000000000'))
  })
})
