...
describe("NFTVotingStrategy:", () => {
  ...
  const baseSetup = deployments.createFixture(async () => {
    ...
    const NFTLinearContract = await ethers.getContractFactory("NFTLinearVoting");
    const nftLinearVoting = await NFTLinearContract.deploy(
      safe.address,
      nft.address,
    ...
    const NFTSingleContract = await ethers.getContractFactory("NFTSingleVoting");
    const nftSingleVoting = await NFTSingleContract.deploy(
      safe.address,
      nft.address,
      proposalModule.address,
      thresholdBalance, // number of votes wieghted to pass
  ...
  describe("timelock", async () => {
    ...
    it("should revert update timelock period if not from avatar/owner linear", async () => {
      const { nftLinearVoting, safe } = await baseSetup();
      await expect(nftLinearVoting.updateTimeLockPeriod(1337)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
    });

    it("can update timelock period from proposal linear", async () => {
      const { proposalModule, safe, nftLinearVoting, nft } =
        await baseSetup();
      ...
      await proposalModule.submitProposal([txHash], nftLinearVoting.address, "0x");
    ...
    it("should revert update timelock period if not from avatar/owner single", async () => {
      const { nftSingleVoting, safe } = await baseSetup();
      await expect(nftSingleVoting.updateTimeLockPeriod(1337)).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
    });

    it("can update timelock period from proposal single", async () => {
      const { proposalModule, safe, nftSingleVoting, nft } =
        await baseSetup();
      await nft.delegate(wallet_0.address);
      await nft.connect(wallet_1).delegate(wallet_1.address);
      const Call = buildContractCall(
      ...
      await proposalModule.submitProposal([txHash], nftSingleVoting.address, "0x");
  ...
  describe("openzepplin nft voting modules", async () => {
    it("can vote past the threshold single", async () => {
      ...
      expect(await nftSingleVoting.hasVoted(0, wallet_0.address)).to.equal(true);
      expect(await nftSingleVoting.hasVoted(0, wallet_1.address)).to.equal(true);
    });
    ...
    it("can vote past the threshold linear", async () => {
      ...
      expect(await nftLinearVoting.hasVoted(0, wallet_0.address)).to.equal(true);
    });
